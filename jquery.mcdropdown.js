		...
		function position($el, bUseOffset){
			...
			var pos = $.extend($el[bUseOffset === true ? "offset" : "position"](),{
				...
				, height: $el.outerHeight()
		                , marginLeft: parseInt($($el[0]).css('marginLeft'),10) || 0
		                , marginRight: parseInt($($el[0]).css('marginRight'),10) || 0
			...
			return (
				// we add "0" to each string to make sure parseInt() returns a number
		                parseInt("0"+$(el).css("border"+torl+"Width"), 10)
		                    + parseInt("0"+$(el).css("border"+borr+"Width"), 10)
		                    + parseInt("0"+$(el).css( "padding"+torl), 10)
		                    + parseInt("0"+$(el).css("padding"+borr), 10)
		                    + parseInt("0"+$(el).css("margin"+torl), 10)
